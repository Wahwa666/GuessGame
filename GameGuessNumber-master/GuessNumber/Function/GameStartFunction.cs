...
namespace PanGamesFunction.Function
{
    public class GameStartFunction
    {
        ...
        public IActionResult RunStartGame(
            [HttpTrigger(AuthorizationLevel.Function, "get", Route = "startgame/{login}/{password}")] HttpRequest req, string login, string password)
        {
            Random random = new Random();
            UserInfo user = new UserInfo();
            user.Login = login;
            user.Password = password;
            user.HiddenNumber = random.Next(1, 100);
